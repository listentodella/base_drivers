/*TP驱动
上报触摸事件
前面几个驱动由于都是用打印语句，比较方便查看效果
这里已经使用input，所以装载完驱动后，查看效果需要：
1. ls /dev/event*
2.insmod s3c_ts.ko
3. ls /dev/event*
4. hexdump /dev/event? ------新增的一般就是tp对应的


此外还可以使用TSLIB库（用于测试TP的），百度即可
...
static void s3c_ts_timer_function(unsigned long data)
{
  if(s3c_ts_regs->adcdat0 & (1 << 15)){//该寄存器的第15bit表示按下或松开,1为松开
    /*已经松开*/
    input_event_abs(s3c_ts_dev, ABS_PRESSURE, 1);//如果是绘图板，会有多级的压感，这里的TP比较简单，因此只有按下松开，1是按下
...
static irqreturn_t adc_irq(int irq, void *dev_id)
{
  ...
  if(s3c_ts_regs->adcdat0 & (1 << 15)) {//如果已经松开
    cnt = 0;
    input_event_abs(s3c_ts_dev, ABS_PRESSURE, 0);//如果是绘图板，会有多级的压感，这里的TP比较简单，因此只有按下松开，1是按下
    ...
    if (cnt == 4) {
      /*优化措施4：软件过滤*/
      if (filter_ts(x, y)) {
        /*将printk换成上报事件就可以成为一个较为完整的TP驱动了*/
        //printk("x = %d, y = %d\n", (x[0] + x[1] + x[2] + x[3]) / 4,
        //(y[0] + y[1] + y[2] + y[3]) / 4);
        input_event_abs(s3c_ts_dev, ABS_X, (x[0] + x[1] + x[2] + x[3]) / 4 );
        input_event_abs(s3c_ts_dev, ABS_Y, (y[0] + y[1] + y[2] + y[3]) / 4 );
        input_event_abs(s3c_ts_dev, ABS_PRESSURE, 1);//如果是绘图板，会有多级的压感，这里的TP比较简单，因此只有按下松开，1是按下
        input_report_key(s3c_ts_dev, BTN_TOUCH, 1);//
        input_sync(s3c_ts_dev);
      }
