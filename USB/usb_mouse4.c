...
3.接上USB鼠标
4.ls /dev/event*
5.cat /dev/tty1 然后按鼠标的键
6.hexdump /dev/event0（新增的event编号）
...
static void usbmouse_as_key_irq(struct urb *urb, struct pt_regs *regs)
{
  ...
  if((pre_val & (1 << 1)) != (usb_buf[0] & (1 << 1))) {
    /*右生了变化*/
    input_event(uk_dev, EV_KEY, KEY_S, (usb_buf[0] & (1 << 0) ? 1 : 0);
    input_sync(uk_dev);
  }

  if((pre_val & (1 << 2)) != (usb_buf[0] & (1 << 2))) {
    /*中键发生了变化*/
    input_event(uk_dev, EV_KEY, KEY_ENTER, (usb_buf[0] & (1 << 0) ? 1 : 0);
    input_sync(uk_dev);
  }
  pre_val = usb_buf[0];
  /*重新提交urb*/
