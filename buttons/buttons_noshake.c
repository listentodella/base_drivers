/*
 * 处理抖动，记住两要素：超时时间和处理函数
 * 超时时间：和闹钟一样，只要重设定时，只有最新的才会生效
 * 
 */
...
static int button_drv_init(void)
{
	...
	//buttons_timer.expires = jiffies + 100*HZ;//如果不写相当于设超时为0
	buttons_function = buttons_timer_function;//两要素之处理函数
	add_timer(&buttons_timer);//将该定时器告诉内核
